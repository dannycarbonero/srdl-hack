...
augmented_subject = 'All'
with open(data_path + 'silver_augmented_data_frame_' + augmented_subject + '.pkl', 'rb') as file:
...
for subject in LOO_subjects:
    ...
    network_directory = get_parent_path('data', subdirectory='Spike Ripples/silver/RippleNet_tuned_augmentations_' + str(epochs) + '_epochs_' + augmented_subject + '/', make=True)

    for factor in augmentation_factors:

        print(f'Tuning on subject %s for %ix the original data'.format(subject, factor))
        ...
        augmented_training_frame_indices = find_dataframe_overlap(training_frame, augmented_data, factor - 1, num_files - 1)
