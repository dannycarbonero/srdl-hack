...
from utilities import binarize_classifications, make_naive_labels, create_training_subset
...
for i, subject in zip(range(len(LOO_subjects)), LOO_subjects):
    ...
    # make labels
    training_labels = np.expand_dims(make_naive_labels(training_classifications, training_series_downsampled.shape[1]), 2)
    validation_labels = np.expand_dims(make_naive_labels(validation_classifications, validation_series_downsampled.shape[1]), 2)
    ...
    with open('RippleNet_tuning_history_subj_' + str(i) +'.pkl', 'wb') as file:
        pickle.dump(history.history, file)

    model.save('RippleNet_tuned_end_subj_' + str(i) + '.h5')
