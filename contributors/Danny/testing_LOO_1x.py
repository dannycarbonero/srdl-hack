...
from utilities import generate_LOO_subjects, pull_event_probabilities, build_data_sets, find_optimum_ROC_threshold, classify_continuous_predictions, calculate_prediction_statistics, create_training_subset, find_dataframe_overlap
...
network_directory = get_parent_path('data', subdirectory = 'Spike Ripples/silver')
...
for subject in LOO_subjects:
    ...
    model = keras.models.load_model(network_directory + 'archive/RippleNet_tuned_augmentations_128_epochs_' + subject + '/RippleNet_tuned_' + subject + '_1x.h5')
    model.summary()

    with open(data_directory + 'silver_augmented_data_frame_' + subject + '.pkl', 'rb') as file:
    ...
    sub_bk_frame = training_frame[training_frame['classification'] == 'bk'].sample(
        n=(training_frame.shape[0] * (factor + 1)) - (training_frame.shape[0] + augmented_training_frame.shape[0]),
        replace=True)
    final_training_frame = pd.concat([training_frame, augmented_training_frame, sub_bk_frame], axis=0)
    testing_frame = final_training_frame
