...
augmentation_factors = [1,5]
num_files = 5

data[['time_start', 'time_stop']] = data[['time_start', 'time_stop']].round(2)

with open(data_path + 'silver_naive_data_frame.pkl', 'rb') as file:
    naive_data = pickle.load(file)
naive_data[['time_start', 'time_stop']] = naive_data[['time_start', 'time_stop']].round(2)
network_directory = get_parent_path('data', subdirectory='Spike Ripples/silver/RippleNet_tuned_naive_' + str(epochs) + '_epochs_final/', make=True)
...
for factor in augmentation_factors:
    ...
    for subject in LOO_subjects:
        ...
        naive_training_frame_indices = find_dataframe_overlap(training_frame, naive_data, factor, num_files)
        naive_training_frame = naive_data.iloc[naive_training_frame_indices, :]
        sub_bk_frame = training_frame[training_frame['classification'] == 'bk'].sample(n = (training_frame.shape[0] * (factor + 1)) - (training_frame.shape[0] + naive_training_frame.shape[0]) , replace=True)
        final_training_frame = pd.concat([training_frame, naive_training_frame, sub_bk_frame], axis = 0)
